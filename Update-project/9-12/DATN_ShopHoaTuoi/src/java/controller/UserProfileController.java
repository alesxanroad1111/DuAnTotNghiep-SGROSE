...
public class UserProfileController {
    ...
    public String updateProfile(ModelMap model, HttpSession httpsession, @ModelAttribute("userr") User user,
            ...
            @RequestParam("avatar2")String avatar2, BindingResult result, @RequestParam("image") MultipartFile image) throws IOException {
        ...
        if (!user.getName().matches("^[a-zA-Z_ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶ"
            ...
            try {
                String path = context.getRealPath("/img/faces/" + image.getOriginalFilename());
                image.transferTo(new File(path));
                avatar2 = image.getOriginalFilename();
