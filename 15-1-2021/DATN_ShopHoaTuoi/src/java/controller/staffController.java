...
// return "redirect:/admin/staff/index";
    ...
    public String delete(HttpSession httpsession, ModelMap model, @PathVariable("id") int id) {
        Session session = factory.getCurrentSession();
        int idd = Integer.parseInt(httpsession.getAttribute("idstaff").toString());
        if(idd!=id){
            Staff staff = (Staff) session.get(Staff.class, id);
            session.delete(staff);    
        }else{
            model.addAttribute("message", "Không thể xóa bản thân!");
    }
